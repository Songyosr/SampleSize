...
SS_calculation2 <- function(outcome = c("Proportion", "Mean", "Incidence Rate"),
                            ...
                            Ec, Ei = NA, # AbsoluteEff = NA, RelativeEff = NA,
                            SDi = NULL, SDc = NULL, m = NA, CVi = NA, CVc = NA,
                            alpha = 0.05, power = 0.8, cluster = FALSE, ceiling_num = FALSE, check = FALSE) {
  # Check inputs
  outcomes <- match.arg(outcome)
  method <- match.arg(method)
  Ec <- as.numeric(Ec)
  if (is.null(SDi)) SDi <- SDc
  if (!cluster && !all(is.na(m), is.na(CVi), is.na(CVc))) {
  ...
  N <- (cluster + (zalpha + zbeta)^2 * ((a + b) / c))
...
df_prepare <- function(outcome = NA, method = NA, Ec = NA, Ei = NA,
                       SDc = NA, SDi = NA, CVc = NA, CVi = NA, m = NA, alpha = 0.05,
                       power = 0.8, cluster = FALSE, ceiling_num = TRUE, check = FALSE,
                       aes_x_col = "Ec", x_step_size = 0, x_step_range = c(-2, 2),
                       aes_color_col = "Ei", col_var = NA, expand = T,...) {
  # Combine all arguments into a list
  arg_list <- lst(
    outcome, method, Ec, Ei, SDc, SDi, CVc, CVi, m, alpha, power, cluster,
    ceiling_num, check, ...
  )

  arg_list[[aes_x_col]] <- parse_x(arg_list[[aes_x_col]], x_step_size,
    from = x_step_range[1],
    to = x_step_range[2]
  )
  print(x_step_size)
  print(arg_list[[aes_x_col]])
